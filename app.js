...
(function () {
	...
	try {
		global.BotConfig = JSON.parse(fs.readFileSync('./config/StormFN/config.json', 'utf8', function (err, data) {
	...
	if (!BotConfig) {
		fs.mkdirSync(`./config/StormFN/`, { recursive: true });
		fs.writeFile(`./config/StormFN/config.json`, JSON.stringify({
	...
	app.listen(port, () => {
		console.log(`Storm++ v${version} is listening on port ${port}!`);
	});
